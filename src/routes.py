...
from models  import db, User, Planet, Character, Vehicle, Post, Favourites #la clase
...
def handle_get_vehicles():
    ...
    vehicles = list(map(lambda vehicleitem : vehicleitem.serialize(), vehicles))
 
    return jsonify(vehicles), 200

@api.route('/vehicles/<int:vehicle_id>', methods=['GET'])
def handle_get_sinlge_vehicle(vehicle_id):

    my_vehicle = Vehicle.query.get(vehicle_id)

    return jsonify(my_vehicle.serialize()), 200
...
def handle_add_vehicle_as_favourite_to_user(vehicle_id):

    my_active_user = User.query.filter_by(is_active=True).first()
    my_searched_vehicle = Vehicle.query.filter_by(id=vehicle_id).first()

    new_favourite = Favourites(
        user_id = my_active_user.id,
        post_id =my_searched_vehicle.post[0].id
    )
    db.session.add(new_favourite)
    db.session.commit()

    return f"Vehicle added to favourites of {my_active_user.username}", 200
...
def handle_delete_favourited_character_from_user(people_id):
    ...
    #encontrar el fav a eliminar
    post_for_item = Post.query.filter_by(character_media_id=people_id).first() 
    target_favourite_to_delete = Favourites.query.filter_by(user_id = my_active_user.id).filter_by(post_id=post_for_item.id).first()
...
def handle_delete_favourited_vehicle_from_user(vehicle_id):

    my_active_user = User.query.filter_by(is_active=True).first()

    #encontrar el fav a eliminar
    post_for_item = Post.query.filter_by(vehicle_media_id=vehicle_id).first() 
    target_favourite_to_delete = Favourites.query.filter_by(user_id = my_active_user.id).filter_by(post_id=post_for_item.id).first()
    ...
    return f"Vehicle deleted from favourites of {my_active_user.username}", 200